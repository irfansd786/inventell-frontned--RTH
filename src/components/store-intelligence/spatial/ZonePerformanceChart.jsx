...
export default function ZonePerformanceChart({ performance = [], zones = [], className = "" }) {
  ...
  const chartData = useMemo(() => {
    ...
    if (zones && zones.length > 0) {
      return zones.map((z) => ({
        zone: z.name,
        traffic: z.visits || z.points || 0,
        dwell: z.avg_dwell_s || z.avgDwellSecs || 45,
        dwellFormatted: z.avg_dwell || z.avgDwell || "45s",
        occupancy: z.people ?? z.count ?? 0,
        trafficShare: parseFloat(z.traffic_share || z.trafficShare) || 0,
      }));
    }
    return [];
  }, [performance, zones]);
