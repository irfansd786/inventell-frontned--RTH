...
export default function CustomerFlowCard({ customerFlow = [], className = "" }) {
  const flows =
    customerFlow.length > 0
      ? customerFlow
      : [
          { path: "Entrance → Grocery → Checkout", count: 142, share: 45 },
          { path: "Entrance → Produce → Grocery", count: 88, share: 28 },
          { path: "Grocery → Produce → Checkout", count: 64, share: 20 },
          { path: "Aisle 1 → Promotional Area → Checkout", count: 48, share: 15 },
        ];
