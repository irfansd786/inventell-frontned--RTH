...
export default function OperationalRecommendations({
  ...
  const [executedIds, setExecutedIds] = React.useState({});
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-lg p-3.5 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                SOURCE: REAL-TIME CCTV ANALYSIS
              </span>
            </div>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              Computer vision-driven actions to minimize wait times
            </p>
          </div>
        </div>
      ...
      {recommendations.length === 0 ? (
        <div className="p-6 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
          <Users className="w-6 h-6 text-slate-400 mx-auto mb-1.5 opacity-50" />
          <p className="text-xs font-semibold text-slate-700 dark:text-slate-200">
            No Critical Queue Bottlenecks Detected
          </p>
          <p className="text-[11px] text-slate-400 mt-0.5">
            Checkout wait times and customer arrivals are currently within SLA thresholds.
          </p>
        </div>
      ) : (
